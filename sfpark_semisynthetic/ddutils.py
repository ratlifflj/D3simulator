...
class ddproblem:
    ...
    def runZOgrad(self,nT_pairs, seeds=[1], VAREPSILON = 0.25, K=1, l_star_old=False):
        if l_star_old:
            l_star=max([max([self.m*theta+qq for qq in self.q_occ]) for theta in np.linspace(-2,2,1000)])
        else:
    ...
    def runZOgrad_(self,street_name, blk, starting_rate=2.00, timewin=(1200,1400), 
                      pre_loaded_dataset=None, print_diagnostics=False, 
                      pre_data=None, pairs=[(1,120), (4,30), (8,15), (15,8)]):
        final_prices = dict()
        GAMMA=1*1e-3
        data = dict()
        ...
        occ = dataset_creator_weekdays_window(street_name + " " +str(blk), 
                                              timewin=timewin, pre_loaded_dataset=pre_loaded_dataset)
        ...
        m = estimate_m_from_final_occupancy_(q, sf_park_ycoords[-1]-starting_rate, end_occ)
        print("m: {}".format(m))
...
def estimate_m_from_final_occupancy_(q, theta, final_occ, N=1e4):
    d = np.asarray(pd.DataFrame.copy(q, deep=True)['OCCUPANCY_FRAC'])
    return (final_occ - np.mean(np.random.choice(d, int(N), replace=True))) / theta
...
def zo_run(street_name, blk, starting_rate=2.00, timewin=(1200,1400), 
                  pre_loaded_dataset=None, print_diagnostics=False, 
                  pre_data=None, pairs=[(1,120), (4,30), (8,15), (15,8)]):
    final_prices = dict()
    GAMMA=1*1e-3
